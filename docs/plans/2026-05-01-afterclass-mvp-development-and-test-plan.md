...
- [x] M7-10 家长 AI：服务有效期查询  
  ...
  **提交记录：** 已提交到本地 Git：`94ea110 feat(ai): add guardian service validity query`；计划状态补充提交 `98519f4 docs(plan): record M7-10 commit status`；GitHub push 已完成。
...
- [x] M7-13 老师 AI：同类题生成  
  ...
  **提交记录：** 已提交到本地 Git：`94a6ab2 feat(ai): add teacher similar question command`；计划状态补充提交 `d2eb895 docs(plan): record M7-13 commit status`；GitHub push 已完成。

- [x] M7-14 管理端 AI：班级核算查询  
  **内容：** 按权限查询校区、班级、毛利。  
...
- [x] M7-17 E2E：AI 确认与拒绝  
  ...
  **提交记录：** 已提交到本地 Git：`1a56efe test(ai): cover confirmation and refusal flow`；计划状态补充提交 `27822ba docs(plan): record M7-17 commit status`；GitHub push 已完成。

---

### M8. 四端 UI 完整落地

- [x] M8-01 将设计 token 写入 Tailwind/theme  
...
- [x] M8-03 管理端 AI 经营助手 UI  
  ...
  **提交记录：** 已提交到本地 Git：`80439a1 feat(ui): improve admin AI assistant`；计划状态补充提交为当前 `docs(plan): record M8-03 commit status`；GitHub push 已完成。
...
- [x] M8-07 家长端首页 UI
  ...
  **提交记录：** 已提交并推送 GitHub：`2bf03ba feat(ui): improve parent home mobile`；计划状态补充提交 `a58d236 docs(plan): record M8-07 completion`。

- [x] M8-08 家长端 AI 助手 UI
...
- [x] M8-11 学生端今日任务 UI
  ...
  **提交记录：** 已提交并推送 GitHub：`271dbe7 feat(ui): add student today tasks page`。
...
- [x] M8-13 全局状态页面
  ...
  **提交记录：** 已提交并推送 GitHub：`4e82a1e feat(ui): add global status showcase`。
...
- [x] M9-02 创建 docker-compose.yml
  ...
  **提交记录：** 已提交并推送 GitHub：`62cc926 build: add docker compose topology`。

- [x] M9-03 创建 `.env.example`
...
- [x] M9-04 创建数据库迁移脚本
  ...
  **提交记录：** 已提交并推送 GitHub：`5d2c5f4 build: add prisma migration deploy scripts`；计划状态补充提交 `f3cf534 docs(plan): record M9-04 commit status`。

- [x] M9-05 创建 demo seed 脚本
  **内容：** 多角色、多校区、完整样例链路。
  **测试：** seed 后可直接演示四端。
...
- [x] M9-08 最终回归测试
  ...
  **提交记录：** 已提交并推送 GitHub：`22dd557 test: add final MVP regression smoke`。
...
- [x] M9-10 商用上线风险清单
  ...
  **提交记录：** 已提交并推送 GitHub：`7b1cd6c docs(ops): add commercial launch risk checklist`。
...
- [x] 发布反馈后家长可见，未发布不可见。
- [x] 发布后错题自动收录。
- [x] AI 生成同类题后老师确认生成 Word。
- [x] 收费记录影响家长服务有效期。
- [x] 班级核算按校区权限隔离。
- [x] AI 操作日志完整记录。

**完成记录：** 2026-05-03 新增 `tests/integration/mvp-integration-checklist-coverage.test.ts`，将 4.2 每一项集成测试清单映射到现有可执行单测/E2E 文件与业务哨兵，确保建档、老师签到、拍照到托通知、匹配失败确认补发、作业批改发布、错题收录、同类题到 Word、收费有效期、班级核算权限和 AI 操作日志追溯都有自动化覆盖。
**验证：** targeted 验证通过：`npm run typecheck && npm run test:integration`（2 个 integration 文件 / 3 个测试通过）；完整质量门禁通过：`npm run typecheck && npm run lint && npm run test:unit && npm run test:integration && npm run prisma:validate && npm run build && npm run test:e2e`（85 个 unit 文件 / 238 个测试通过，2 个 integration 文件 / 3 个测试通过，50 个 E2E 通过）。
...
- [x] 老师端今日托管签到流程。
- [x] 学生拍照签到 → 家长收到到托通知。
- [x] 老师上传作业 → AI 圈错建议 → 老师发布 → 家长查看。
- [x] 错题本 → 同类题 → Word 练习单。
- [x] 收费录入 → 家长服务有效期展示 → 管理端班级核算。
- [x] 家长 AI 请假确认。
- [x] 老师 AI 反馈草稿确认。
- [x] 管理端 AI 查询班级毛利。
- [x] 管理端 AI 高风险修改收费被拒绝。

**完成记录：** 2026-05-03 复核 4.3 E2E 测试清单，`tests/e2e` 已覆盖基础建档、老师今日托管/拍照签到、家长到托通知、作业 AI 确认发布、错题到练习单、收费与核算可见性、家长请假确认、老师反馈草稿、管理端 AI 核算查询和高风险收费修改拒绝。
**验证：** `npm run test:e2e` 已在完整质量门禁中通过，50 个 Playwright 用例全部通过。
