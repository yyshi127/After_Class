...
export function ParentHomeworkFeedbackDetail({
  ...
  return (
    <section className="rounded-neu bg-surface p-6 shadow-neu-sm" aria-labelledby="parent-homework-detail-heading">
      ...
      <div className="mt-6 rounded-2xl bg-surfaceAlt p-5 shadow-neu-inset">
        ...
        <div className="mt-4 grid gap-3">
          {detail.attendanceTimeline.map((record) => (
            <article key={record.id} className="rounded-2xl bg-background p-4 shadow-neu-sm">
              ...
              {record.photoFileId ? <p className="mt-1 text-sm text-muted">签到照片：{record.photoFileId}</p> : null}
            </article>
          ))}
        </div>
      </div>
